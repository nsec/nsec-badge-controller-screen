...
esp_err_t send_pong(uint16_t addr, uint64_t ping_idx)
{
    ...
    ESP_LOGI(TAG, "Sending pong in response to node=0x%02x idx=%llu", addr, ping_idx);

    err = esp_ble_mesh_server_model_send_msg(srv_vnd_model, &ctx, OP_VND_PONG, sizeof(pong), (uint8_t *)&pong);
	if (err != ESP_OK) {
        ESP_LOGI(TAG, "%s: Send pong failed (err %u)", __func__, err);
        return err;
    }
...
esp_err_t pong_received(esp_ble_mesh_model_t *model, esp_ble_mesh_msg_ctx_t *ctx, struct net_buf_simple *buf)
{
    ...
    ESP_LOGI(TAG, "Received pong from node=0x%02x mac=" MAC_STR_FMT " idx=%llu",
        pong->src_node_addr, MAC_BYTES(pong->src_mac_addr), pong->idx);
